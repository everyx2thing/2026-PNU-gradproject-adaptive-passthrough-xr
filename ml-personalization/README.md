...
- 지금 모든 결과는 **mock 데이터 기반**이라, RF 모델 accuracy(97%)는 라벨이 feature로부터 파생된 구조라서 나온 결과, 실제데이터로 재검증 전까지는 무의미 (정상 동작 확인용)
- `mock_*.csv`, `*.joblib`, `*.onnx` 파일은 `.gitignore` 처리되어 있어서 저장소에는 안 올라감 + 필요하면 스크립트 재실행으로 언제든 다시 생성
